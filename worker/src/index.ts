...
const unsafeSignals = [
  { pattern: /(скажу|назову|сообщу|продиктую|отправлю|введу).{0,24}(код|смс|парол|данн)/, tag: 'код-подтверждения' },
  { pattern: /(переведу|отправлю).{0,24}(деньг|сумм|оплат)/, tag: 'перевод-денег' },
  ...
  { pattern: /(продолжу|соглашусь|сделаю|подтвержу)/, tag: 'давление-срочностью' },
]
...
const evaluateUserReplyHeuristically = (userMessage: string): UserReplyEvaluation | null => {
  ...
  if (safePatterns.some((pattern) => pattern.test(normalized))) {
    return {
      userWasSafe: true,
      userVerdict:
        'Да, это безопасная реакция: вы не поддались давлению и решили проверить информацию через независимый канал.',
      mistakeTag: null,
    }
  }
  ...
  if (unsafeMatch) {
    return {
      userWasSafe: false,
      userVerdict:
        'Нет, это рискованный ответ: вы начинаете действовать по правилам мошенника вместо независимой проверки.',
...
const detectCodeRequest = (assistantReply: string) =>
  /(код|смс|sms|одноразов|парол)/i.test(assistantReply)
  ...
  return {
    ...
    userVerdict:
      typeof data.userVerdict === 'string'
        ...
        : 'Оценка ответа не была сформирована.',
    userWasSafe: Boolean(data.userWasSafe),
...
Верни ТОЛЬКО JSON-объект без markdown и без дополнительного текста:
{
  ...
  "userVerdict": "одно короткое предложение, правильно ли пользователь ответил и почему",
  "userWasSafe": true,
...
- userVerdict должен быть коротким и прямым: правильно ли пользователь ответил и почему.
- userWasSafe=true, если пользователь не поддался на давление, отказался сообщать код, решил проверить источник, прекратить диалог или не переводить деньги.
...
- если userWasSafe=false, укажи mistakeTag коротким тегом вроде "код-подтверждения", "давление-срочностью", "переход-по-ссылке", "перевод-денег", "слабая-проверка";
- если userWasSafe=true, верни mistakeTag=null.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    return json(
      {
        ...normalized,
        userWasSafe: heuristicEvaluation?.userWasSafe ?? normalized.userWasSafe,
        userVerdict: heuristicEvaluation?.userVerdict ?? normalized.userVerdict,
        mistakeTag: heuristicEvaluation?.mistakeTag ?? normalized.mistakeTag,
        simulatedCode:
